...
export class EditorToolbar extends EventEmitterMixin(DOMElement) {
  ...
  #build() {
    ...
    const save = Utils.ui.button('Save');
    const revert = Utils.ui.button('Revert');
    const dirty = Utils.buildDOM(['span', {
      'data-dirty': 'true'
    }, this.dirty ? 'Unsaved changes' : 'Saved']);
    this.addDOMEventListener(save, 'click', () => this.emit('save', EditorEvent.fromObject({
      kind: 'save-requested'
    })));
    this.addDOMEventListener(revert, 'click', () => this.emit('revert', EditorEvent.fromObject({
      kind: 'revert-requested'
    })));
    this.node.append(/* save, */ revert, dirty);
  }
}
